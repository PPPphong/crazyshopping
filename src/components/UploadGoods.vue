...
export default {
  setup() {
    ...
    const openConfirmationDialogSuccess = () => {
      ElMessageBox.alert('商品信息上传成功', '成功', {
    ...
    const openConfirmationDialogFail = () => {
      ElMessageBox.alert('商品信息上传失败', '失败', {
    ...
    const submitForm = async () => {
      ...
      try {
        const response = await axios.post('http://localhost:8080/goods', formData, {"Content-Type": "multipart/form-data"});
